...
def codigo_todo():
        ...
        #####################
        camin_FNET = ''
        #camin_FNET = 'C:/Users/joaovictor/OneDrive - FUNCEF/Documentos/João/Robozinho_de_noticias'    
        ...
        para_quem_vou_enviar = ['GFUNCEF_automatizacofii@funcef.com.br' , "reinaldo@funcef.com.br", "joaovictor@funcef.com.br"]

        ## DF 
        #########

        # Ler o arquivo, tratando a coluna de CNPJ como string
        excel = pd.read_csv(camin_excel, sep=",", dtype={'CNPJ': str})

        # Preencher zeros à esquerda na coluna de CNPJ para garantir que tenha 14 dígitos
        excel['CNPJ'] = excel['CNPJ'].apply(lambda x: x.zfill(14))
        ...
        print('-'*100)
        print('-'*100)
        print('TABELA ENVIO DE E-MAIL - Resumindo notícias...')
        #df_email["noticia"] = df_email["id"].apply(lambda x: resumo_da_noticia(x))
        #df_email["noticia"] = df_email["id"].apply(lambda x: x)
        print('TABELA ENVIO DE E-MAIL - COM os Resumos')
        print(df_email)
        print('-'*100)
